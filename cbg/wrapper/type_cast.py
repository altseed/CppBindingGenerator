...
T = TypeVar('T')
def _type_cast(self:WrapperGenerator, type_:T, name:str):
    if type_ in [ctypes.c_byte, int, float, bool, ctypes.c_wchar_p, ctypes.c_void_p] + self.definition.structs:
        return name
    elif type_ in self.definition.classes:
        func_name = self.shared_ptr_creator_name
        type_name = type_name._get_cpp_fullname(type_)
        return '{0}<{1}>(({1}*){2})'.format(func_name, type_name, name)
    elif type_ in self.definition.enums:
...
U = TypeVar('U')
def _return_cast(self:WrapperGenerator, type_:T, name:str):
    if type_ in [ctypes.c_byte, int, float, bool, ctypes.c_wchar_p, ctypes.c_void_p]:
        ...
        type_name = type_name._get_cpp_fullname(type_)
        return '{0}<{1}>(({1}*){2})'.format(func_name, type_name, name)
    elif type_ in self.definition.structs:
        return '({})'.format(name)
    elif type_ in self.definition.enums:
        return '(int32_t){}'.format(name)
    for dependency in type_.dependencies:
        if type_ in self.definition.classes:
