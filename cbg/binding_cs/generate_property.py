...
def _generate_unmanaged_property(self:BindingGeneratorCS, code:Code, prop:Property, class_:Class, definition:Definition):
    if prop.has_getter:
        self._generate_unmanaged_func(code, prop.getter_as_func(), class_, definition)
    if prop.has_setter:
        self._generate_unmanaged_func(code, prop.setter_as_func(), class_, definition)

BindingGeneratorCS._generate_unmanaged_property = _generate_unmanaged_property

def _generate_managed_property(self:BindingGeneratorCS, code:Code, prop:Property, class_:Class, definition:Definition):
    ...
    # プロパティ本体
    name = self._get_cs_type(prop.type_, definition)
    ...
    with CodeBlock(code, '{} {} {}'.format(access, name, prop.name), IndentStyle.BSDAllman):
        ...
        if prop.has_getter:
            with CodeBlock(code, 'get'):
                ...
                self._write_managed_function_body(code, prop.getter_as_func(), class_, definition)
        # セッタ
        if prop.has_setter:
            with CodeBlock(code, 'set'):
                ...
                self._write_managed_function_body(code, prop.setter_as_func(), class_, definition)
    # 内部変数
