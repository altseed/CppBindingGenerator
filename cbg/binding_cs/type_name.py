...
# エイリアスを取得
T = TypeVar('T')
def _get_alias_or_name(type_:T, definition:Definition):
    namespace = ''
...
# 入力された型をC#形式で文字列で取得
U = TypeVar('U')
def _get_cs_type(type_:U, definition:Definition, called_by:ArgCalledBy = None):
    ptr = ''
    ...
    if isinstance(type_, Class): return _get_alias_or_name(type_, definition)
    if isinstance(type_, Struct): return ('' if called_by == None else ptr) + type_.alias
    if isinstance(type_, Enum): return _get_alias_or_name(type_, definition)
    if type_ is None: return 'void'
    raise ValueError("{} is not supported in cs.".format(str(type_)))

# 入力された型をC言語形式で文字列で取得
V = TypeVar('V')
def _get_csc_type(type_:V, definition:Definition, called_by:ArgCalledBy = None):
    ptr = ''
